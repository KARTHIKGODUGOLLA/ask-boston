...
CLI:  python -m baseline.naive_rag "Which Peña spoke at the May 19 committee meeting?"
    ...
    if not any(docs_dir.glob("doc*.md")):
        ...
        if full is not None:  # a known edition — split its corpus in place
            ...
            raise SystemExit(
                ...
                f"  python -m baseline.split_corpus --input <corpus.md> --outdir {docs_dir}"
            )
...
def main() -> None:
    ...
    if not question:
        raise SystemExit('Usage: python -m baseline.naive_rag "your question"')
